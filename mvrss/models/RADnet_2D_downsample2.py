"""
- For addressing 64x64x32 tensor
"""
...
class ASPPBlock(nn.Module):
    ...
    def __init__(self, in_ch, out_ch=256):
        super().__init__()
        self.global_avg_pool = nn.AvgPool2d((32, 32))
    ...
    def forward(self, x):
        x1 = F.interpolate(self.global_avg_pool(x), size=(32, 32), align_corners=False,
                           mode='bilinear')
...
class EncodingBranch(nn.Module):
    ...
    def forward(self, x):
        ...
        x2 = self.single_conv_block1_1x1(x1_down)
        # return input of ASPP block + latent features
        return x1_down, x2


class RADnet_2D_downsample2(nn.Module):
    """ 
    ...
    def forward(self, x_rd, x_ra, x_ad):
        ...
        x7_rd = self.rd_double_conv_block2(x6_rd)
        x7_ra = self.ra_double_conv_block2(x6_ra)

        # Final 1D convolutions
        x8_rd = self.rd_final(x7_rd)
        x8_ra = self.ra_final(x7_ra)
