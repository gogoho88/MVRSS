...
for sequence in annotations.keys():
    save_path_radmat = os.path.join('/data/datasets_master/Carrada_RAD/',sequence,'mod2_RAD_numpy')
    save_path_annot = os.path.join(path,sequence,'mod2_annotations','dense')
    ...
    for template in annotations[sequence]:
        ...
        if downsampling>0:             # downsample or interpolation
            ...
            zi = np.linspace(0,rad_mat.shape[2]-1,rad_mat.shape[2]//(downsampling//2))
            X,Y,Z = np.meshgrid(xi,yi,zi)
            rad_mat_resize = f_interp3d((Y,X,Z))    # 이상하게 이렇게 해야 원래랑 같아짐..
            # Mask (RD, RA)
            rd_mask_resize = rd_mask[:,::downsampling,::(downsampling//2)]
